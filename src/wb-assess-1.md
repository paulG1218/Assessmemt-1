...
- [x] **C.** The code causes an error because it will output `['JS', 'HTML', 'CSS']` three times.
- [ ] **D.** There is no error; the code generates the expected output.
...
```
console.log will print an output to your console, but return will only give an output and it needs other code to use that output.
```

**_What's the difference between defining and calling a function?_**

```
Defining a function creates the function and therefore allows dthe function to be called. Calling a function is when you tell the function to run its code.
```
